...
public class Torre_Transito {
	...
	public boolean reporte_transito(){  //el a es un array con las calles
		try{
			...
			Date d = new Date();
			d.getDate();
			inter.informe_torre("Informe de transito a las: "+d.toString());
			for(int i =0;i<this.ciu.obtener_calles().size();i++){
				aux = this.ciu.obtener_calles().get(i);
				x = (int)(Math.random()*10);
				aux.setDificultad(x);
				inter.informe_torre("La Calle "+aux.getId()+" ha cambiado su dificultad a: "+x);
			}
	...
	public boolean dificultad_calle(long id,int dificultad){
		if(dificultad>=0){
			...
			inter.informe_torre("La Calle "+id+" ha cambiado su dificultad a: "+dificultad);
			this.ciu.getHospital().recalculo_ruta();
			return true;
		}else{
			return false;
		...
		if(dificultad>=0){
			...
			while(it.hasNext()){
				...
				if((aux.getInicio().getId()==id_ini) && (aux.getFin().getId()==id_fin)){
					...
					inter.informe_torre("La Calle "+aux.getId()+" ha cambiado su dificultad a: "+dificultad);
					this.ciu.getHospital().recalculo_ruta();
					return true;
				}
			}

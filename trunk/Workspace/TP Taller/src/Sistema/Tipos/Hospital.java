...
public class Hospital{
				...
				if(ped.tipo_transporte().getSuperclass()!=Aereos.class){
					...
					if(llego){
						...
						this.interfaz.informar("El movil: "+ped.get_ambulancia().getId()+" atendiendo al pedido "+ped.getId()+" ha llegado al punto: "+ped.get_ambulancia().getPos_actual(),  
                                                        Interfaz.InformarConst.INFORME_SIMULACION);
		...
		try{
			...
			if(resp.equalsIgnoreCase("SI")){
				...
				if(resp.equalsIgnoreCase("NO")){
					this.interfaz.informar("El pedido "+ped.getId()+" ha sido rechazado", Interfaz.InformarConst.INFORME_SIMULACION);
	...
	public void recalculo_ruta(){
		Iterator<Pedido> it = this.pedidos_activos.iterator();
		Pedido aux;
		this.interfaz.informar("Recalculando Rutas", Interfaz.InformarConst.INFORME_SIMULACION);
		...
		try{
			if(!this.estacion.hay_ambulancias(ped.tipo_transporte())){ //si no hay estaciones
				if(ped.en_limbo()){//si el estado del pedido es en el limbo
					...
					this.interfaz.informar("El pedido "+ped.getId()+" no puede ser atendido en este momento, desea encolarlo?", Interfaz.InformarConst.PEDIDO_ESPERA);
					this.log.log_normal("EN ESPERA A RESPUESTA: "+ped.getId()+", Destino: "+ped.get_destino());
					...
					this.interfaz.informar("El pedido "+ped.getId()+" ha sido encolado", Interfaz.InformarConst.INFORME_SIMULACION);
				...
				if(this.cola_pedidos.get(ped.tipo_transporte()).isEmpty()){ //si hay ambulancias y la cola esta vacia
					...
					this.interfaz.informar("El pedido "+ped.getId()+" ha sido despachado", Interfaz.InformarConst.INFORME_SIMULACION);
					...
					this.interfaz.informar("El pedido "+p.getId()+" ha sido despachado ya que tenia mayor prioridad.", Interfaz.InformarConst.INFORME_SIMULACION);
					if(ped.en_limbo()){//si el estado del pedido es en el limbo
						this.pedidos_dudosos.add(ped); //lo meto en dudosos e informo
						this.interfaz.informar("El pedido "+ped.getId()+" no puede ser atendido en este momento, desea encolarlo?", Interfaz.InformarConst.PEDIDO_ESPERA);
	...
	private void llego_destino(Pedido ped){
		...
		this.interfaz.informar("El movil: "+ped.get_ambulancia().getId()+" atendiendo al pedido "+ped.getId()+" ha llegado al destino: "+ped.get_destino(), Interfaz.InformarConst.INFORME_SIMULACION);
	...
	private void llego_hospital(Pedido ped){
		...
		this.interfaz.informar("El pedido: "+ped.getId()+" Ambulancia: "+ped.get_ambulancia()
				+"Destino Original: "+ped.get_destino()+", volvio al hospital exitosamente", Interfaz.InformarConst.INFORME_SIMULACION);
